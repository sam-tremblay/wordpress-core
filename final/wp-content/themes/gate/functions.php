	...
	function __construct(){
		...
		*/
		add_action('after_setup_theme', function(){
			...
			/*
			* If ACF Pro not found or not activated
			*/

			if(
				!in_array($pagenow, ['plugins.php', 'themes.php'])
				...
				wp_die(
					__('L\'extension "Advanced Custom Fields PRO" est manquante ou n\'est pas activée.'),
					__('Une erreur est survenue'),
					[
						'link_url' => admin_url('plugins.php'),
						'link_text' => __('Se rendre sur la page des extensions')
					]
				);
		...
		*/
		add_action('wp_enqueue_scripts', function(){
			...
			*/
			...
			if(gc::field('global_styles') !== 'enable')
				wp_dequeue_style('global-styles');
			
			if(gc::field('wp_block_library') !== 'enable')
				wp_dequeue_style('wp-block-library');
			
			if(gc::field('classic_theme_styles') !== 'enable')
		...
		*/
		...
		if(is_dir($this->acf_path)){
			add_filter('acf/settings/save_json', function($path){
				return get_stylesheet_directory() . '/datas/acf';
			});
			...
			add_filter('acf/settings/load_json', function($paths){
				...
				// Append our new path
				$paths[] = get_stylesheet_directory() . '/datas/acf';
		...
		*/
		add_action('template_redirect', function(){
			...
			if(!is_front_page() && gc::field('maintenance') === 'enable' && !in_array($userRole, ['administrator'])){
		...
		*/
		add_action('wp_head', function(){
			...
			if(gc::field('seo_management') === 'disable') return;
			...
			if(
				...
				(
					...
					!gc::field('index_se', 'user_' . get_queried_object()->ID)
				)
				...
				(
					...
					!gc::field('index_se', get_queried_object()->taxonomy . '_' . get_queried_object()->term_id)
				)
			)
			...
			*/
			if(is_author()){
				...
				$title = $author->first_name . ' ' . $author->last_name . ' - ' . (gc::field('global_seo_title_se') ? gc::field('global_seo_title_se') : get_bloginfo('name'));

				if(gc::field('title_se', 'user_' . $author->ID))
					$title = gc::field('title_se', 'user_' . $author->ID);

				if(gc::field('description_se', 'user_' . $author->ID))
					$description = gc::field('description_se', 'user_' . $author->ID);

				if(gc::field('title_sn', 'user_' . $author->ID))
					$og_title = gc::field('title_sn', 'user_' . $author->ID);

				if(gc::field('description_sn', 'user_' . $author->ID))
					$og_description = gc::field('description_sn', 'user_' . $author->ID);

				if(gc::field('image_sn', 'user_' . $author->ID))
					$og_image = gc::field('image_sn', 'user_' . $author->ID);

			}
			elseif(is_tax() || is_tag() || is_category()){
				$term = get_queried_object();
				
				$title = $term->name . ' - ' . (gc::field('global_seo_title_se') ? gc::field('global_seo_title_se') : get_bloginfo('name'));

				if(gc::field('title_se', $term->taxonomy . '_' . $term->term_id))
					$title = gc::field('title_se', $term->taxonomy . '_' . $term->term_id);

				if(gc::field('description_se', $term->taxonomy . '_' . $term->term_id))
					$description = gc::field('description_se', $term->taxonomy . '_' . $term->term_id);

				if(gc::field('title_sn', $term->taxonomy . '_' . $term->term_id))
					$og_title = gc::field('title_sn', $term->taxonomy . '_' . $term->term_id);

				if(gc::field('description_sn', $term->taxonomy . '_' . $term->term_id))
					$og_description = gc::field('description_sn', $term->taxonomy . '_' . $term->term_id);

				if(gc::field('image_sn', $term->taxonomy . '_' . $term->term_id))
					$og_image = gc::field('image_sn', $term->taxonomy . '_' . $term->term_id);
			...
			* Title
			*/
			if($normal && !empty(gc::field('title_se')))
				$title = gc::field('title_se');

			elseif($normal && !empty(gc::field('global_seo_title_se')))
				$title = get_the_title() . ' - ' . gc::field('global_seo_title_se');


			/*
			* Description
			*/
			if($normal && !empty(gc::field('description_se')))
				$description = gc::field('description_se');

			elseif($normal && !empty(gc::field('global_seo_description_se')))
				$description = gc::field('global_seo_description_se');


			/*
			* og:title
			*/
			if($normal && !empty(gc::field('title_sn')))
				$og_title = gc::field('title_sn');

			elseif($normal && !empty(gc::field('global_seo_title_sn')))
				$og_title = gc::field('global_seo_title_sn');


			/*
			* og:description
			*/
			if($normal && !empty(gc::field('description_sn')))
				$og_description = gc::field('description_sn');

			elseif($normal && !empty(gc::field('global_seo_description_sn')))
				$og_description = gc::field('global_seo_description_sn');


			/*
			* og:image
			*/
			if($normal && !empty(gc::field('image_sn')))
				$og_image = gc::field('image_sn');
			...
			if(gc::field('favicons_seo_internet_explorer'))
				$html .= '<!--[if IE]><link rel="shortcut icon" href="'. gc::field('favicons_seo_internet_explorer') .'"><![endif]-->';

			if(gc::field('favicons_seo_apple_touch'))
				$html .= '<link rel="apple-touch-icon" sizes="180x180" href="'. gc::field('favicons_seo_apple_touch') .'">';

			if(gc::field('favicons_seo_all_browsers_and_android'))	
				$html .= '<link rel="icon" sizes="192x192" href="'. gc::field('favicons_seo_all_browsers_and_android') .'">';

			if(gc::field('favicons_seo_msapplication_tileimage'))
				$html .= '<meta name="msapplication-TileImage" content="'. gc::field('favicons_seo_msapplication_tileimage') .'">';
		...
		*/
		add_action('admin_init', function(){
			...
			* Clean Dashboard
			*/
			if(gc::field('clean_dashboard') === 'enable'){
			...
			if(
				gc::field('change_display') === 'enable'

				&&

				!empty(gc::field('restrict_gate_tab'))

				&&

				!in_array(wp_get_current_user()->ID, gc::field('restrict_gate_tab'))

				&&

				(
					(isset($_GET['page']) && $_GET['page'] === 'gate-settings')

					||

					(isset($_GET['post_type']) && in_array($_GET['post_type'], ['acf-field-group', 'is_gate_cpt']))

					||

					(isset($_GET['post']) && in_array(get_post_type($_GET['post']), ['acf-field-group', 'is_gate_cpt']))
		...
		*/
		add_action('admin_bar_menu', function(){
			...
			if(gc::field('change_display') === 'enable'){
				...
				*/
				...
				if(current_user_can('edit_theme_options') && !empty(gc::field('register_nav_menus')))
					$wp_admin_bar->add_node($args);
				...
				*/
				...
				if(
					...
					*/
					...
					if(gc::field('cpt_management') !== 'disable')
						$wp_admin_bar->add_node($args);
					...
					*/
					$args = array(
						'id' => 'is-gate-themes-editor',
						'title' => __('Editor'),
						'href' => $admin_url . 'theme-editor.php',
						'parent' => 'is-gate-themes',
						'meta' => array(
							'class' => 'is-gate-themes-editor'
						)
					);
					...
					*/
					$args = array(
						'id' => 'is-gate-plugin-editor',
						'title' => __('Éditeur', 'is-gate-core'),
						'href' => $admin_url . 'plugin-editor.php',
						'parent' => 'is-gate-plugins',
						'meta' => array(
							'class' => 'is-gate-plugins-editor'
						)
					);
		...
		*/
		add_action('admin_head', function(){
			...
			if(gc::field('change_display') === 'enable' || !empty(gc::field('restrict_gate_tab')))
				echo '<style type="text/css">#toplevel_page_gate-settings{display: none !important;}</style>';
		...
		*/
		add_action('init', function(){
			...
			if(gc::field('cpt_management') !== 'disable'){
				$change_display = gc::field('change_display');
				/*
				* Add CPT Manager
				*/
				$labels = array(
			...
			*/
			...
			acf_add_local_field([
				...
				'label' => 'Restrict Gate Tab',
				'name' => 'restrict_gate_tab',
				'aria-label' => '',
				'type' => 'user',
			...
			if(gc::field('seo_management') !== 'disable') {
				...
				*/
				$post_types = !empty(gc::field('post_types_seo')) ? gc::field('post_types_seo') : [];
			...
			*/
			$menus = !empty(gc::field('register_nav_menus')) ? gc::field('register_nav_menus') : [];
			...
			*/
			$options_pages = !empty(gc::field('register_options_pages')) ? gc::field('register_options_pages') : [];
			if($options_pages){
